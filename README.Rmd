...
the [rentrez] package, but parsing the XML can be challenging. The `tidypubmed`
...
`entrez_search` and `entrez_fetch` in [rentrez] and will parse the results
...
et al.  The untidy author string is also include in `pubmed_table` above.
...
There are an number of additional nodes you may want to parse in the [PubMed] XML.
